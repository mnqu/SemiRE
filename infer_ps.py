...
for i, batch in enumerate(iterator_unlabeled):
    ...
    selector.eval()
    pred = torch.sigmoid(selector.predict(inputs)) * 100
    pred = pred.data.cpu().numpy().tolist()
    preds_s += pred
...
for eid, pred in ranking:
    ...
    if rel not in rel2preds:
        rel2preds[rel] = []
    rel2preds[rel] += [(eid, pred)]
...
for rel in range(len(RELATION.vocab)):
    ...
    ranking = list(zip(range(num_instance), preds_s))
    ranking = sorted(ranking, key=lambda x:x[1][rel], reverse=True)

    #fot = open('temp/output_' + str(rel) + '.txt', 'w')

    for eid, pred in rel2preds.get(rel, []):
        example = examples[eid]
        true = RELATION.vocab.stoi[example.relation]

        if preds_s[eid][nolid] > 50:
            predrel = nolid
            continue
        else:
            predrel = rel
        ...
        #fot.write('true: {}, {:.4f}, {:.4f}\tpred: {:.4f}, {:.4f}\tnone: {:.4f}, {:.4f}\n'.format(true, pred[true], preds_s[eid][true], pred[rel], preds_s[eid][rel], pred[nolid], preds_s[eid][nolid]))
        ...
        output['subj_pst'] = example.subj_pst
        output['obj_pst'] = example.obj_pst
        output['relation'] = RELATION.vocab.itos[predrel]
        fo.write(json.dumps(output) + '\n')
